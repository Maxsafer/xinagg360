...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="XInput -> single virtual X360 aggregator")
    parser.add_argument("--poll-hz", type=int, default=250, help="Polling rate (Hz)")
    parser.add_argument("--idle-hold-ms", type=int, default=500, help="Owner hold after last activity (ms)")
    parser.add_argument("--deadzone", type=int, default=6000, help="Analog stick deadzone (0..32767)")
    parser.add_argument("--trigger-deadzone", type=int, default=5, help="Trigger deadzone (0..255)")
    ...
    try:
        ...
        pass  # not available on some environments

    print("[xinagg360] starting… Ctrl+C to quit")
    agg.start()

    # Park the main thread while the background thread polls
    try:
        while True:
            time.sleep(1.0)
